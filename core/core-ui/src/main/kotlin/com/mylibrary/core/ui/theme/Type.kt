 ...
 *
 * **3. The interface is set in the platform's own stack.** Noto Naskh Arabic / Noto Sans Arabic for
 * Arabic, Roboto for Latin — chosen by Android for the script being drawn, which is the arrangement
 * that cannot get the script wrong. The bundled faces belong to the *reader*, where [ReaderFont]
 * offers them for a document's text; the app's own chrome is not a place a face is chosen for.
 */
